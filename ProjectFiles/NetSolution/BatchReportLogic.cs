...
public class BatchReportLogic : BaseNetLogic
{
    ...
    public void QueryReportData()
    {
        ...
        var temperatureMax = Owner.GetVariable("TemperatureMax");
        var temperatureMin = Owner.GetVariable("TemperatureMin");
        var HumidityMax = Owner.GetVariable("HumidityMax");
        var HumidityMin = Owner.GetVariable("HumidityMin");
        ...
        myStore.Query("SELECT MAX(Temperature) FROM BatchDataLogger WHERE BatchID = '" + batchID.Value + "'", out Header, out ResultSet);
        temperatureMax.Value = Convert.ToSingle(ResultSet[0, 0]);
        myStore.Query("SELECT MIN(Temperature) FROM BatchDataLogger WHERE BatchID = '" + batchID.Value + "'", out Header, out ResultSet);
        temperatureMin.Value = Convert.ToSingle(ResultSet[0, 0]);
        myStore.Query("SELECT MAX(Humidity) FROM BatchDataLogger WHERE BatchID = '" + batchID.Value + "'", out Header, out ResultSet);
        HumidityMax.Value = Convert.ToSingle(ResultSet[0, 0]);
        myStore.Query("SELECT MIN(Humidity) FROM BatchDataLogger WHERE BatchID = '" + batchID.Value + "'", out Header, out ResultSet);
        HumidityMin.Value = Convert.ToSingle(ResultSet[0, 0]);
